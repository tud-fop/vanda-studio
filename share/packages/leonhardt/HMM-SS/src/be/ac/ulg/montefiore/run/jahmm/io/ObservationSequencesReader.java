...
{	
	...
	throws IOException, FileFormatException
	{
		...
		for (st.nextToken(); st.ttype != CustomStreamTokenizer.TT_EOF; 
		st.nextToken()) {
			st.pushBack();
	...
	readSequence(ObservationReader<O> oir, CustomStreamTokenizer st) 
	throws IOException, FileFormatException
	{	
		for (st.nextToken(); st.ttype == CustomStreamTokenizer.TT_EOL;
		st.nextToken());
		if (st.ttype == CustomStreamTokenizer.TT_EOF)
			return null;
		...
		do {
			...
			if (st.nextToken() == '\\') { /* New lines can be escaped by '\' */
				if (st.nextToken() != CustomStreamTokenizer.TT_EOL)
