...
{	
	...
	//EDITED BY CHRISTOF LEONHARDT
	//Value for check of zero
	...
	{		
		...
		for (int i = 0; i < hmm.nbStates(); i++) {
			...
			//EDITED BY CHRISTOF LEONHARDT
			//Check for zero was not sufficient
			//if (aijDen[i] == 0.) // State i is not reachable
			if (Math.abs(aijDen[i]) < EPSILON || Double.isNaN(aijDen[i])){
				for (int j = 0; j < hmm.nbStates(); j++)
		...
		for (int o = 0; o < sequences.size(); o++)
			for (int i = 0; i < hmm.nbStates(); i++) {
				...
				//EDITED BY CHRISTOF LEONHARDT
				//No check for NaN and check for zero was not sufficient
				//if (aijDen[i] == 0.) // State i is not reachable
				if (Math.abs(nhmm.getPi(i)) < EPSILON && !Double.isNaN(allGamma[o][0][i])){
					nhmm.setPi(i, nhmm.getPi(i) + allGamma[o][0][i]);
				} else if (!Double.isNaN(allGamma[o][0][i])) { 
					nhmm.setPi(i,
		...
		for (int i = 0; i < hmm.nbStates(); i++) {
			...
			//EDITED BY CHRISTOF LEONHARDT
			//No check for NaN and check for zero was not sufficient
			if (Math.abs(sum) > EPSILON && !Double.isNaN(sum)) { 
				for (j--; j >= 0; j--)
					weights[j] /= sum;
