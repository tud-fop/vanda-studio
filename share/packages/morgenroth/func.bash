...
Bleu () {
	mkdir -p "$1/reference"
	ln -s -T "$3" "$1/reference/r1"
	"$BLEU/bleu" -m "$2" -r "$1/reference" | grep "BLEU" | sed "s/BLEU score: //g" > "$4"
...
Bleu4 () {
	mkdir -p "$1/reference"
	ln -s -T "$3" "$1/reference/r1"
	ln -s -T "$4" "$1/reference/r2"
	ln -s -T "$5" "$1/reference/r3"
	ln -s -T "$6" "$1/reference/r4"
	"$BLEU/bleu" -m "$2" -r "$1/reference" | grep "BLEU" | sed "s/BLEU score: //g" > "$7"
	unlink "$1/reference/r1"
