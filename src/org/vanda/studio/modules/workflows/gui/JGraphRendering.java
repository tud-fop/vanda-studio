...
public class JGraphRendering {
	...
	public static void render(Hyperworkflow to, Graph g, Object parentCell) {
	...
	protected static mxStylesheet createStylesheet() {
		...
		for (Renderer r : renderers) {
			style = new HashMap<String, Object>(stylesheet
	...
	protected static interface Renderer {
		...
		void render(Hyperworkflow to, Graph g, Object parentCell);
	...
	protected abstract static class DefaultRenderer implements Renderer {
		...
		public void render(Hyperworkflow hwf, Graph g, Object parentCell) {
			...
			try {
				...
				mxCell v = (mxCell) g.insertVertex(parent, null, hwf, hwf
						.getX(), hwf.getY(), hwf.getWidth(), hwf.getHeight(),
						this.getStyleName());
		...
		public void render(Connection c, Graph g, Object parentCell) {
			...
			try {
				...
				// check if source or target equal the parent (i.e. inner ports
				// are used)
				if (parent instanceof mxCell
						&& ((mxCell) parent).getValue() != null
						&& ((mxCell) parent).getValue().equals(c.getSource())) {
					source = (mxICell) parent;
					innerSource = true;
				}
				if (parent instanceof mxCell
						&& ((mxCell) parent).getValue() != null
						&& ((mxCell) parent).getValue().equals(c.getTarget())) {
				...
				for (Object o : childVertices) {
					// check if vertice's values equal the source or target of
					// the connection
					if (!innerSource
							&& ((mxCell) o).getValue().equals(c.getSource()))
						source = (mxICell) o;
					if (!innerTarget
							&& ((mxCell) o).getValue().equals(c.getTarget()))
						target = (mxICell) o;
				}
	...
	protected static class Graph extends mxGraph {
		...
		public String convertValueToString(Object cell) {
			...
			if (value instanceof Hyperworkflow)
				return ((Hyperworkflow) value).getName();
			else
				return "";
		}
		...
		public Object createVertex(Object parent, String id, Object value,
			...
			mxCell vertex = new mxCell(value, geometry, style) {
				...
				protected Object cloneValue() {
					...
					if (value instanceof Hyperworkflow) {
						try {
							return ((Hyperworkflow) value).clone();
		...
		public boolean isValidSource(Object cell) {
			return super.isValidSource(cell)
					&& (cell == null || ((mxCell) cell).getValue() instanceof Port
